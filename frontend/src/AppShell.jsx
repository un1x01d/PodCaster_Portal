...
export default function App() {
  ...
  const { sortedData } = React.useMemo(() => {
    ...
    if (activeCols.length > 0) {
          ...
          if (allowed instanceof Set) {
            // Case-insensitive exact match for multi-select/chat-set filters
            if (allowed.size > 0) {
              let matched = false;
  ...
  const handleSwitchSheet = (newSheetId, selectedName = "") => {
    ...
    // Force a fresh load when switching revisions from the top picker.
    // This avoids stale-view/cache behavior where selection appears to no-op.
    setSelectedViewId("");
    hydrateSheetContext(newSheetId, { preserveFilters: false, preferCache: false });
  };
