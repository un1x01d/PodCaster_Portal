...
export async function resolveMetricHeaders({ metricKey, headers = [], fieldMetadata = {}, message = "", sampleRows = [] }) {
  ...
  for (const canonicalField of req.required) {
    const out = await resolveField({ canonicalField, headers, fieldMetadata, message, resolvedMappings, sampleRows });
    if (out.status === "resolved") {
      ...
      ambiguous.push({ canonicalField, ...out });
    } else {
